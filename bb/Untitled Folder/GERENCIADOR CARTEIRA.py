...
def update_table_filter():
    for linha in con.cursor().execute('''SELECT cred FROM moviment WHERE cod = (SELECT MAX(cod) FROM moviment)'''):
        lista = linha
    return lista
...
def validacao_valor(valor, mod):
    ...
    if val == 's' or val == 'S':
        ...
        if cred < 3.10 and cred >= 1.55:
            ...
            print('ATENÇÃO!!! VOCÊ NÃO TEM MAIS NENHUMA PASSAGEM! (integrado)')
            if cred >= 1.35:
                print('ATENÇÃO!!! VOCÊ SÓ TEM MAIS UMA PASSAGEM! (não-integrado)')
            else:
                print('ATENÇÃO!!! VOCÊ NÃO TEM MAIS NENHUMA PASSAGEM! (não-integrado)')
